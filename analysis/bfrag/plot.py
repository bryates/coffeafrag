...
for key in output:
    ...
    for iax,ax in enumerate(list(output[key].axes[0])):
        gkey = ax
        year = ''
        if gkey not in data_list:
            year = '_1' + gkey.split('_1')[1]
...
def plot_mass(meson='d0'):
    ...
    for var in ['l0pt', 'j0pt', 'b0pt', 'nleps', 'njets', 'nbjets', 'jet_flav', 'ht']:
        ...
        h_name = [s for s in out.axes['dataset'] if 'TTToSemiLep' not in s and 'TTTo2L2Nu' not in s and not any(d in s for d in data_list)]
...
def plot_control(meson='d0', var_lst = ['l0pt', 'j0pt', 'b0pt', 'nleps', 'njets', 'nbjets', 'jet_flav', 'ht', 'met', 'nvtx']):
    pdgId = mass_id[meson] if meson in mass_id else 421
    #if meson == 'qcd': pdgId = 0
    meson_name = meson
    meson = meson.replace('mu', '')
    #var_lst = ['l0pt']
    for var in var_lst:
        if meson == 'qcd' and var == 'b0pt': continue
        if var not in output:
            print(f'Skipping {var}!')
            continue
        ...
        print(h_name)
        h_samp = [histo for _,_,histo in h_samp]
        tt_ll = h[{'dataset': ttbar_di}][{'dataset': sum}]
        tt_ln = h[{'dataset': ttbar_semi}][{'dataset': sum}]
        #h_samp.append(h[{'dataset': ['TTTo2L2Nu_16', 'TTTo2L2Nu_16APV']}][{'dataset': sum}])
        #h_samp.append(h[{'dataset': ['TTToSemiLeptonic_16', 'TTToSemiLeptonic_16APV']}][{'dataset': sum}])
        #h_samp.append(h[{'dataset': ttbar_di}][{'dataset': sum}])
        #h_samp.append(h[{'dataset': ttbar}][{'dataset': sum}])
        ...
        h_name.append('$t\overline{t} \\rightarrow l$')
        if meson == 'qcd':
            h_qcd = h3 - h_mc - tt_ll - tt_ln
            #print(f'{h3.values()=}, {(h3-tt_ll-tt_ln).values()=}, {h_qcd.values()=}, {(mc_tot-tt_ll-tt_ln).values()=}, {h_mc.values()=}')
            q = h_qcd.values()
            q[q<0] = 0
            q = np.nan_to_num(q, 0)
            h_qcd[...] = q
            ...
            if var != 'b0pt':
                h_qcd = plot_control('qcd', [var])
                #h_qcd = h_qcd *  np.sum(mc_tot.values())
                #h_qcd = h_qcd * np.sum((tt_ll + tt_ln).values()) / np.sum(mc_tot.values())
                h_qcd = h_qcd / np.sum(h_mc.values())
        #h_samp.append(h_qcd)
        #h_name.append('QCD')
        hep.histplot(h3, yerr=True, label='Data', histtype='errorbar', density=dens, color='k', flow='none')
        hep.histplot(h_samp, label=h_name, stack=True, histtype='fill', density=dens, flow='none')
        ...
        if len(hs.axes['systematic']) > 1:
            ...
            syst_up = hs[{'systematic': 'nominal'}].values() + syst_up + h_qcd.values()
            syst_down = hs[{'systematic': 'nominal'}].values() - syst_down + h_qcd.values()
            #plt.stairs(syst_up, baseline=syst_down, label='Total syst.', **error_band_args)
            plt.fill_between(hs.axes[var].edges[1:], syst_down, syst_up, label='Total syst.', **error_band_args)
        plt.legend()
        plt.xlabel('')
        ...
        if var != 'b0pt':
            mc_tot += h_qcd
        hep.histplot(h3/(h_mc+h_qcd+tt_ll+tt_ln), yerr=np.sqrt(h3.values())/np.sum(h3.values()), label='Data/MC', histtype='errorbar', density=dens, color='k', flow='none')
        ...
        plt.close()
        if meson == 'qcd':
            return h_qcd / np.sum((mc_tot - h_qcd).values())
...
#plot_control('qcd', ['l0pt'])
plot_control('ttbar', ['l0pt', 'njets'])
#plot_control('d0', ['nleps', 'l0pt', 'njets'])
#plot_control('d0', ['ht', 'nleps', 'l0pt'])
#plot_mass('d0')
#plot_mass('jpsi')
#plot_mass('d0mu')
