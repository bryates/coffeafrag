...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(self, samples, hist_lst=None, ecut_threshold=None, do_errors=False, do_systematics=False, split_by_lepton_flavor=False, skip_signal_regions=False, skip_control_regions=False, muonSyst='nominal', dtype=np.float32):
        ...
        lep_axis = hist.axis.StrCategory(name="lep_cat", label="Top channel", categories=[], growth=True)
        ...
        self.output = processor.dict_accumulator({
            ...
            'l0pt': hist.Hist(dataset_axis, meson_axis, lep_axis, lpt_axis, cut_axis, systematic_axis),
            ...
            'nleps' : hist.Hist(dataset_axis, meson_axis, lep_axis, nleps_axis, cut_axis, systematic_axis),
    ...
    def process(self, events):
        ...
        frag = uproot.open(coffeafrag_path('data/bfrag/bfragweights.root'))
        ...
        ele["isTightLep"] = b_os.isTightElec(ele)
        mu["isTightLep"]  = b_os.isTightMuon(mu)
        ele["isVetoLep"]  = b_os.isVetoElec(ele)
        mu ["isVetoLep"]  = b_os.isVetoMuon(mu)
        ele = ele[b_os.isTightElec(ele)]
        mu  = mu[b_os.isTightMuon(mu)]
        #ele = ele[b_os.isTightElec(ele) & ~b_os.isVetoElec(ele)]
        #mu  = mu[b_os.isTightMuon(mu) & ~b_os.isVetoMuon(mu)]
        ...
        b_es.add1lMaskAndSFs(events, year, isData, 'prompt')
        ...
        for syst_var in syst_var_list:
            ...
            jets["isGood"] = b_os.is_tight_jet(jets.pt, jets.eta, jets.jetId, pt_cut=30., eta_cut=get_b_param("eta_j_cut"), id_cut=get_b_param("jet_id_cut"))
            cleanedJets["isGood"] = b_os.is_tight_jet(getattr(cleanedJets, jetptname), cleanedJets.eta, cleanedJets.jetId, pt_cut=30., eta_cut=get_b_param("eta_j_cut"), id_cut=get_b_param("jet_id_cut"))
            ...
            if not isData:
                ...
                bJetSF   = GetBTagSF(goodJets, year, 'medium')#FIXME
                ...
                if self._do_systematics and syst_var=='nominal':
                    for b_syst in ["bc_corr","light_corr",f"bc_{year}",f"light_{year}"]:
                        bJetSFUp = GetBTagSF(goodJets, year, 'MEDIUM', syst=b_syst)[0]#FIXME
                        bJetSFDo = GetBTagSF(goodJets, year, 'MEDIUM', syst=b_syst)[1]#FIXME
                ...
                # Trigger SFs
                #GetTriggerSF(year,events,leptons,leptons[:,0],leptons[:,1])
            ...
            for ch_name in ["ttbar", "d0", "d0mu", "jpsi", "qcd"]:
                ...
                '''
                if not isData:
                    weights_dict[ch_name].add("lepSF_muon", mu.sf_nom_1l_muon, copy.deepcopy(mu.sf_hi_1l_muon), copy.deepcopy(mu.sf_lo_1l_muon))
                    #weights_dict[ch_name].add("lepSF_elec", ele.sf_1l_elec, copy.deepcopy(ele.sf_1l_hi_elec), copy.deepcopy(ele.sf_1l_lo_elec))
            ...
            ptzmask = tc_es.get_Z_peak_mask(leptons,15,flavor="os")
            ...
            sr_cat_dict = {
                "ttbar" : {
                    "cuts" : events.is_ttbar & ~ptzmask & b_mask & pass_trg & cand_mask,# & chi2_mask,
                },
                "d0" : {
